...
const ProblemDescription: React.FC<ProblemDescriptionProps> = ({
  ...
  const { liked, disliked, starred, solved, setData } =
    useGetUsersDataOnProblems(problem.id);
  const returnUserDataAndProblems = async (transaction: any) => {
  ...
  const handlelike = async () => {
    ...
    await runTransaction(firestore, async (transaction) => {
      const { userDoc, problemDoc, userRef, problemRef } =
        await returnUserDataAndProblems(transaction);
      ...
      <div className="flex px-0 py-4 h-[calc(100vh-94px)] overflow-y-auto">
        <div className="px-5">
          ...
          <div className="w-full">
            ...
            {!loading && currentProblems && (
              <div className="flex items-center mt-3">
                <div
                  className={`${problemDifficultyClass} inline-block rounded-[21px] bg-opacity-[.15] px-2.5 py-1 text-xs font-medium capitalize `}
                >
                  {currentProblems.difficulty}
                </div>
                {(solved || _solved) && (
                ...
                >
                  ...
                  <span className="text-xs">{currentProblems.likes}</span>
                </div>
                <div
                ...
                >
                  ...
                  <span className="text-xs">{currentProblems.dislikes}</span>
                </div>
                <div
                  onClick={handleStar}
              ...
              {problem.examples.map((example, index) => (
                <div key={example.id}>
                  ...
                  <div className="example-card">
                    <pre>
                      <strong className="strong">Input: </strong>
                      {example.inputText}
                      <br />
...
function useGetCurrentProblem(problemId: string) {
  const [currentProblems, setCurrentProblems] = useState<DBProblem | null>(
    null
  );
  ...
  useEffect(() => {
    const getCurretProblems = async () => {
      setLoading(true);
      const docRef = doc(firestore, "problems", problemId);
      const docSnap = await getDoc(docRef);
      if (docSnap.exists()) {
        const problem = docSnap.data();
        setCurrentProblems({ id: docSnap.id, ...problem } as DBProblem);
        setProblemDifficultyClass(
          problem.difficulty === "Easy"
            ? "bg-olive text-olive"
            : problem.difficluty === "Medium"
...
function useGetUsersDataOnProblems(problemId: string) {
